...
class Pannel {
	...
	constructor(divpannelid: HTMLDivElement) {
		...
		this.rightpannel = <HTMLDivElement>(
			document.getElementById('rightpannel')
		);
	...
	showdescription(text, id) {
		document.getElementById(
			'pannel' + id
		).innerHTML = `<div id="description">${text}</div>`;
	}
	...
	showscore(text, id) {
		document.getElementById(
			'pannel' + id
		).innerHTML += `<div id="score">Score: ${text}</div>`;
	...
	addButtonToRightPanel(text: string, fun_name, id) {
		...
		this.button.setAttribute(
			'style',
			'position: absolute; bottom: 12vh; width: 90%;'
		);
