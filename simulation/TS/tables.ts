...
class Verify_Table {
	...
	row_ind: number;
	all_orignial_values: number[];
	after_verify;

	constructor(
		header: string[],
		all_data: number[][],
		verify_row_index: number,
		verify_col: number[],
		all_original_values: number[],
		table_title: string,
		outer_div: HTMLElement,
		update_values: boolean,
	...
	load_table() {
		...
		for (let i = 0; i < this.verification_row.length; i++) {
			if (this.verify_col.indexOf(i) == -1) {
				...
				vr_elements += `<td><input class='form-control' id='tab${
					this.tab_ind
				}inp${input_count++}' /></td>`;
			}
		}
		...
		setTimeout(() => {
			let btn: HTMLButtonElement = <HTMLButtonElement>(
				document.getElementById(`tab${this.tab_ind}btn`)
			);
	...
	verify_table = () => {
		...
		for (let i = 0; i < this.verify_col.length; i++) {
			let val: HTMLInputElement = <HTMLInputElement>(
				document.getElementById(`tab${this.tab_ind}inp${i}`)
			);
			if (
				!verify_values(
					parseFloat(val.value),
					this.all_orignial_values[i]
				)
			) {
		...
		if (this.update_values) {
			for (let j = 0; j < this.verify_col.length; j++) {
				let val: HTMLInputElement = <HTMLInputElement>(
					document.getElementById(`tab${this.tab_ind}inp${j}`)
				);
				this.all_data[this.row_ind][this.verify_col[j]] = parseFloat(
					val.value
				);
			}
		}
	...
	load_full_table = () => {
		let tab_body: HTMLTableElement = <HTMLTableElement>(
			document.getElementById(`body${this.tab_ind}`)
		);
		tab_body.innerHTML = ``;

		let lastcol: HTMLTableCellElement = <HTMLTableCellElement>(
			document.getElementById(`tab${this.tab_ind}lastcol`)
		);
		lastcol.remove();
	...
	get_table_element() {
		let ele: HTMLDivElement = <HTMLDivElement>(
			document.getElementById(`tab${this.tab_ind}main`)
		);
		return ele;
	}
}
...
class Verify_Rows_Cols extends Verify_Table {
	rows_verification: number[][] = [];
	original_values: number[][] = [];
	verify_row_index: number[];
	show_data: boolean;
	full_table_load: boolean;
	rows_to_verify: number = 0;
	constructor(
		header: string[],
		all_data: number[][],
		verify_row_index: number[],
	...
	load_table() {
			...
			for (let j = 0; j < this.rows_verification[i].length; j++) {
				if (this.verify_col[i].indexOf(j) == -1) {
					vr_elements += `<td>${
						this.show_data
							? parseFloat(
									this.rows_verification[i][j].toFixed(3)
							  )
							: '--'
					}</td>`;
				} else {
      ...
      `;
		...
		setTimeout(() => {
			let btn: NodeListOf<HTMLButtonElement> = document.querySelectorAll(
				`#tab${this.tab_ind}btn`
			);
			btn.forEach((ele) => (ele.onclick = this.verify_rows));
		}, 200);
	}

	verify_rows = (e: MouseEvent) => {
		console.log('verify button clicked');
		...
		for (let i = 0; i < this.verify_col[row_indx].length; i++) {
			let val: HTMLInputElement = <HTMLInputElement>(
				document.getElementById(`tab${this.tab_ind}inp${row_indx}${i}`)
			);

			if (
				!verify_values(
					parseFloat(vals[i]),
					this.original_values[row_indx][i]
				)
			) {
	...
	load_full_table = () => {
		...
		if (this.full_table_load) {
			...
			for (let i = 0; i < this.all_data.length; i++) {
				...
				for (let j = 0; j < this.verification_row.length; j++) {
					let cell = new_row.insertCell(j);
					cell.innerHTML = parseFloat(
						this.all_data[i][j].toFixed(3)
					).toString();
				}
			}
		}
					...
					vr_elements += `<td>${
						this.show_data
							? typeof this.rows_verification[i][j] == 'number'
								? parseFloat(
										this.rows_verification[i][j].toFixed(
											this.custom_fix
										)
								  )
		...
		for (let i = 0; i < this.verify_col[row_indx].length; i++) {
			...
			if (
				!verify_values(
					parseFloat(vals[i]),
					parseFloat(
						this.original_values[row_indx][i].toFixed(
							this.custom_fix
						)
					)
				)
				...
				for (let j = 0; j < this.verification_row.length; j++) {
					...
					cell.innerHTML =
						typeof this.all_data[i][j] == 'number'
							? parseFloat(
									this.all_data[i][j].toFixed(this.custom_fix)
							  ).toString()
							: this.all_data[i][j];
				}
			}
...
class Verify_Rows_Cols_Custom_Fixed_Update1 extends Verify_Rows_Cols_Custom_Fixed {
	...
	constructor(
		...
		for (let i = 0; i < this.verify_col.length; i++) {
			if (this.verify_col[i].length != 0) this.rows_to_verify++;
		}
	}
}
...
class Show_Table {
	...
	constructor(header: any[], body: any[], outer_div: HTMLElement) {
		this.header = header;
		this.body = body;
		this.outer_div = outer_div;
	}
	...
	load_table() {
		...
		for (let i = 0; i < this.body.length; i++) {
			...
			for (let j = 0; j < this.body[i].length; j++) {
				body_element += `<td>${parseFloat(
					this.body[i][j].toFixed(3)
				)}</td>`;
			}
...
class Show_Table_Custom_Fixed extends Show_Table {
	...
	load_table() {
		...
		for (let i = 0; i < this.body.length; i++) {
			...
			for (let j = 0; j < this.body[i].length; j++) {
				body_element += `<td>${parseFloat(
					this.body[i][j].toFixed(this.fixed)
				)}</td>`;
			}
		...
		if (this.full_table_load) {
			...
			for (let i = 0; i < this.mixed_data.length; i++) {
				...
				for (let j = 0; j < this.total_column; j++) {
					let cell = new_row.insertCell(j);
					cell.innerHTML = `${this.set_num_string(
						this.mixed_data[i][j]
					)}`;
				}
			}
